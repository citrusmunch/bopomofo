...
from .dictionary import pinyin_bopomofo as _dict

# Create an 'xpinyin' instance
_pinyin = Pinyin()
...
def bopomofo_to_pinyin(bopomofo, splitter=" ", tones="marks", default_tone=1):
    ...
    return splitter.join(
        [
            _single_bopomofo_to_pinyin(x, tones, default_tone, ignore_warning=True)
            for x in bopomofos
        ]
    )
...
def _bopomofo_list(chars, tones=None, first_tone_symbol=False):
    ...
    return [
        _single_pinyin_to_bopomofo(x, tones, first_tone_symbol, ignore_warning=True)
        for x in pinyin
    ]
...
def _single_pinyin_append_tone(pinyin, tone):
    ...
    if tone != 0:
        ...
        if m is None:  # pragma: no cover
            ...
            t = (
                t[: m.start(0)]
                + _dict["tones"]["pinyin"][tone][
                    _dict["tones"]["pinyin"][0].index(m.group(0))
                ]
                + t[m.end(0) :]
            )
        else:
